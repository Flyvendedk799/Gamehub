...
describe('buildRenderItems', () => {
  ...
  it('gives the coalesced group a stable key based on its start index', () => {
    const tool: SseEvent = { type: 'tool_use', runId: 'r', toolName: 'edit', status: 'start', timestamp: 't' };
  ...
  it('separates two text runs split by a non-text event', () => {
    const tool: SseEvent = { type: 'tool_result', runId: 'r', toolName: 'edit', success: true, timestamp: 't' };
  ...
  it('passes non-text events through as event items with stable keys', () => {
    ...
    const complete: SseEvent = { type: 'run_complete', runId: 'r', snapshotPath: '', previewUrl: '/p', timestamp: 't' };
