...
2. **\`choose_engine\`** — emit \`{ engine: 'three' | 'phaser' | 'canvas2d', rationale: 1-sentence }\`. Match to brief AND to the spec from step 1:
   ...
   - **RECOMMENDED SKILLS — \`import_skill\`, do NOT retype.** \`choose_engine\` returns the vetted, engine-correct skills that implement YOUR declared capabilities — e.g. \`enemy-ai\` + \`wave-spawner\` for an escalating shooter, \`level-orchestrator\` + \`save-state\` for progression, \`dialog-flow\` for narrative, \`economy-system\` for a shop/tower-defense, \`procedural-gen\` for roguelikes, \`mobile-controls\` for touch. For each, call **\`import_skill({ name })\`** — it writes the tested module to \`src/engine/\` and returns the import line + its exported functions. Then \`import\` it in your entry file and CALL those functions. Do NOT \`view_game_feel\` these and re-type your own version: re-deriving a wave/enemy/progression/dialogue system by hand is the dominant source of flat, buggy, over-long builds, and the imported module already wires its \`getState()\` into \`window.__game.debug.snapshot\` for verification. (\`view_game_feel\` is still right for the small inline FEEL snippets — screen-shake, hitstop, particle-burst — which you DO adapt into your handlers.)
