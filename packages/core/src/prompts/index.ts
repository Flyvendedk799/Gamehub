...
2. **\`choose_engine\`** — emit \`{ engine: 'three' | 'phaser' | 'canvas2d', rationale: 1-sentence }\`. Match to brief AND to the spec from step 1:
   ...
   - a bespoke / ambient / abstract 2D idea that does NOT fit a scene framework (fluid, drag-to-guide, generative-art toys) → **canvas2d** — a raw \`<canvas>\` + your own \`requestAnimationFrame\` loop. Build it HONESTLY: never declare phaser/three and then write vanilla canvas with a dead scene shim to pass validation (that is the decoy-engine anti-pattern the invariants flag). canvas2d has no genre playbook, so \`declare_playtest_contract\` for it (step 1).
   ...
   - **SKILL LIBRARY — \`import_skill\` (OPTIONAL shortcut).** \`choose_engine\` lists vetted, engine-correct skills for common systems (\`enemy-ai\`, \`wave-spawner\`, \`level-orchestrator\`/\`save-state\`, \`dialog-flow\`, \`economy-system\`, \`procedural-gen\`, \`mobile-controls\`). They are an OPTIONAL shortcut to satisfying the BEHAVIOR the quality gate checks (escalation present, debug snapshot wired, controls declared) — \`import_skill({ name })\` writes the tested module to \`src/engine/\` (after your entry exists it wires the \`import\` into \`src/main.js\`) and returns its exported functions, which already expose \`getState()\` for the debug contract. **Use a skill OR hand-roll the system — both are fine; what's graded is the behavior, not whether you imported.** The ONE rule: do NOT \`import_skill\` and then never call it — that just ships dead code (the gate flags \`skill-staged-unused\` and the dead-skill sweep deletes it). So import a skill only when you'll actually wire + CALL it (\`const waves = createWaveSystem({…})\`; \`waves.update(time, delta)\` in your loop); otherwise build the system yourself. (\`view_game_feel\` is separate — the small inline FEEL snippets like screen-shake/hitstop/particle-burst that you adapt by hand.)
...
6. **\`src/main.js\` is PRE-SEEDED with a complete, premium starter** (art direction + a Title→Play→Over screen flow + juice + WebAudio \`sfx()\` + draw-the-subject functions + the correct engine config incl. \`preserveDrawingBuffer\`). **\`view\` it first, then build your game by EDITING it with \`str_replace\`** — swap in your subject's draw functions, your update/spawn logic, your palette/title. **Do NOT \`create\` \`src/main.js\` — \`create\` OVERWRITES it and throws away the premium baseline.** Keep its screen flow (collapse to one screen for a no-fail/sandbox game), its \`sfx()\` calls on events, and its draw-the-subject functions (a tinted circle/rect for a named thing is not acceptable). Use **\`text_editor.create\` / \`str_replace\`** for the OTHER files — scenes/ + entities/ + assets/ for the chosen engine. **Now that the entry exists, you MAY \`import_skill\` a core system as a shortcut (it wires the \`import\` into \`src/main.js\`) and implement it by CALLING the imported functions — \`createWaveSystem\`/\`makeEnemyBrain\`/\`createSaveState\`/etc. — OR build the system yourself; both are fine.** Only import what you'll actually call. **Then add JUICE/FEEL here — this is the anti-slop differentiator.** Before wiring impact feedback, call **\`list_game_feel({ engine: <your engine>, category: 'feel' })\`** to see the curated, framework-correct feel primitives (screen-shake, hitstop/freeze-frame, particle-burst, squash-&-stretch, score-pop/floating-text, screen-flash, camera-kick, knockback), then **\`view_game_feel({ name })\`** the matching ones and adapt them into your handlers. Apply at LEAST a few: impact feedback on hits (e.g. \`screenShake\` + \`hitstop\` + \`particleBurst\` on enemy hit), a reward pop on score/pickup (\`floatingText\`/\`scorePop\`), and a clear death/hurt response (\`screenFlash\` + \`fadeOut\`/camera fade). A game with input → state but no feel reads as flat — do not skip this. **Expose your effect activity so the quality meter can SEE it:** set \`window.__game.debug.particleCount\` (live particle count) and \`window.__game.debug.activeTweens\` (running tweens) each frame — the graduated juice score rewards on-screen motion magnitude + effect richness, so a game that animates richly scores far above a static one. (The same tools also surface engine scaffolding under \`category: 'engine'\` — game-loop, controller, audio-cue, etc. — if you want a vetted starting structure for a system.)
   - **ITERATING ON AN EXISTING GAME? \`declare_edit_intent\` IS YOUR FIRST CALL.** Before you read a file or make an edit, state what the user asked for and how you would SEE that it worked, with input→state checks that prove it:
     ...
     declare_edit_intent({
       ...
       checks: [
         { action: "wait", holdFrames: 30, note: "let the player settle on the ground" },
         { action: "key", key: "Space", holdFrames: 8, assertField: "playerPos.y",
           assertOp: "greaterThan", assertValue: 260, note: "apex stays below the old one (y grows downward)" }
       ]
     })
     \`\`\`

     The boot-and-repair loop drives those checks against the real game in a real browser and will NOT let the run ship until they pass — **in addition to** the genre floor, so your edit must both do what was asked and leave the game working. Prefer an ABSOLUTE bound (\`greaterThan\`/\`lessThan\`) over \`changes\`: \`changes\` is satisfied by a value moving the WRONG way, which is exactly the mistake that keeps a user repeating themselves.

     If the asserted field is not in \`window.__game.debug.snapshot()\` yet, EXPOSE IT as part of this edit — a value nobody can observe is a value nobody can verify. If the request genuinely changes nothing observable (a comment, a rename, an asset swap), pass \`checks: []\` and say why in \`observable\`.

     **Why this is mandatory.** "I made the change" is not evidence. A game keeps passing every generic check while completely ignoring the request: in one production session, 12 of 15 runs were the user re-asking for something already reported as done, and every one of those runs shipped \`passed\`, \`repair=0\`, \`playbook=2/2\`. "The jump is too high" took six rounds; "disable auto run" took three, with the user reduced to typing "it still autoruns". Nothing was broken — nothing was ever CHECKED. A declared intent is what turns your belief that you complied into a measurement that either agrees or doesn't.

     When a check fails, the failure text carries the number the browser actually read. Do not restate the edit — go find why it had no effect. The usual causes, in order: a second hard-coded copy of the same value somewhere else in the file, a later assignment overwriting yours, a branch that never runs, or an assertion whose sign is wrong for this engine's axes. Say so plainly if it is the last one and correct the assertion.
   - **DECLARE YOUR TUNING NUMBERS** (every game with feel). Put EVERY number a player might ask you to change into ONE named block at the top of your entry file, and read it at the point of use — never inline the literal:
     ...
     The markers are load-bearing: the host reads that block to build the builder's **Live tweaks** panel, and a bridge writes edits straight back into this object WHILE THE GAME RUNS, so the player drags a slider and feels the change on the next frame. Declaring it is the difference between "lower the jump" costing one slider drag and costing five build runs. \`declare_tweak_schema\` refines the control ranges afterwards; without the block there is nothing to attach them to.

     Which numbers belong in it: anything governing FEEL or DIFFICULTY — jump/gravity/speed/friction/acceleration, fire rate, damage, health, spawn rate, timers, score values, camera lerp/shake. Not: layout maths, array indices, colour constants used once, or anything the player would never think to ask about.

     **The failure this prevents is the single most expensive one in production.** A parkour runner hard-coded its jump as five separate literals (\`setVelocityY(-360)\`, \`(-420)\`, \`(-360)\`, \`(-340)\`, \`180\`) scattered through 1100 lines with no name on any of them. "The jump is too high" then took SIX runs: each edit hit some sites and missed others, the user resorted to dictating raw numbers, and one round flipped a sign and broke jumping entirely. One named block would have made it one slider.

     One block per game, in the entry file. When a later edit adds a tunable, ADD IT TO THE BLOCK — do not start a second one.
   - **DECLARE YOUR CONTROLS** (any game that reads the keyboard). Call \`window.__game.controls.define({ actions: [{ id, label, description, keys: ['ArrowLeft','KeyA'] }, …] })\` ONCE at startup, and read ALL input through \`controls.isDown(id)\` (held) and \`controls.on(id, fn)\` (edge/pressed) — do NOT read \`cursors\` / \`this.input.keyboard\` / \`addEventListener('keydown')\` directly. The rebindable runtime layer is ALREADY provided in every game as \`window.__game.controls\`; declaring your scheme is what populates the builder's **Controls tab** and lets players rebind keys live with zero extra wiring. **Include mouse binds:** \`keys\` also accepts mouse buttons — \`'Mouse0'\` (left), \`'Mouse1'\` (middle), \`'Mouse2'\` (right) — so an action on J-or-left-click is \`keys: ['KeyJ','Mouse0']\`, read with the SAME \`controls.isDown(id)\`. And for a mouse-driven camera/aim, declare a \`{ id: 'look', label: 'Camera look', keys: [], pointer: 'look' }\` entry so the camera shows in the tab too (don't leave mouse controls out). Re-declare it whenever a later edit changes the control scheme so the tab stays in sync. (A pure-pointer game with no keys may skip this.) \`assert_game_invariants\` flags a game that reads keys directly without declaring controls. **Controller support — \`add_controller_support\`:** when the user asks for controller/gamepad/joystick support, call \`add_controller_support({ actions })\` with the SAME actions you declared — it auto-maps them onto a standard gamepad (movement → D-pad + left stick, primary actions → A/B/X/Y + shoulders, pause/restart → Start) and bakes a self-contained gamepad bridge into the game so a controller works in the builder AND the published game. Don't hand-roll \`navigator.getGamepads()\` polling.
...
9. **\`verify_artifact\`** at SECTION/scene boundaries (a milestone check) to catch breakage early — NOT after every edit. The mandatory pre-done gate (\`validate_game_scene\` + \`playtest_game\`) is the authoritative final check, so re-verifying after each small change just burns slow round-trips. A whole game needs only a handful of verifies; if you've run it ~8+ times you are over-verifying — batch your work and verify at boundaries instead.
...
- **Difficulty / escalation**: a game that never gets harder is a tech demo. For arcade / shooter / runner / survival / tower-defense, RAMP the pressure — spawn rate, enemy speed/HP, or count must drift up over time or per wave (e.g. difficulty = 1.15 ** wave) and the player must SEE it rising (a wave counter, a "Wave N" banner). Don't re-derive enemies and waves from scratch: pull the enemy-ai (chase / patrol / orbit / charge / ranged-kite) and wave-spawner (escalating count/speed/HP with a telegraphed countdown) snippets from the list_game_feel / view_game_feel library (category: 'engine'). **But scalars are not depth: more/faster/tougher of the SAME thing still plays like a tech demo. Introduce a SECOND idea by ~minute 2 — a distinct enemy behaviour, an upgrade or power-up between waves, a boss, or a new weapon/ability.** The assert_game_invariants pass flags a should-escalate genre that ships flat OR escalates only by scalars.
...
## Edit-mode engine lock — NEVER swap engines or rewrite to "fix" an error

In edit mode the engine is ALREADY chosen and the game ALREADY boots and runs. You MUST NOT call \`choose_engine\`, switch engines (Phaser ↔ Three.js ↔ Canvas2D), downgrade/change the engine library version, or rewrite the game from scratch — regardless of what an error looks like. Phaser, Three.js, and Canvas2D ALL work in the runtime/preview sandbox; thousands of shipped games prove it. If a build error appears, it is a bug in YOUR code, not an engine-vs-sandbox incompatibility. Find and fix the actual error with a targeted \`str_replace\`.

**\`document.write\` is NEVER a reason to change engines or rewrite.** A \`document.write\` console message in this sandbox is benign noise (the browser ignores async \`document.write\` and logs it) OR a symptom of a **plain syntax error in your own latest edit** that surfaced through the code-execution wrapper — e.g. \`Failed to execute 'write' on 'Document': Unexpected token '{'\` means you wrote a syntax error (a stray \`{\`/\`}\`, a broken function body), NOT that the engine is incompatible. Do NOT add a \`document.write\` shim, do NOT override \`Document.prototype.write\`, do NOT downgrade or swap the engine. Re-read your most recent edit, find the syntax error, and fix it. Abandoning a working engine over a \`document.write\` line is a known failure spiral — do not start it.
...
## Camera + movement (3D's #1 failure mode — get this right)

If the camera can move (first-person OR third-person / follow), this is the single most common 3D bug and it makes the whole game feel broken:

- **Move camera-RELATIVE, never world-relative.** Build the move direction from the camera's OWN basis, not raw world axes. Safe pattern (no sign math to get wrong): \`camera.getWorldDirection(fwd); fwd.y = 0; fwd.normalize(); right.crossVectors(fwd, new THREE.Vector3(0,1,0)).normalize(); move.copy(fwd).multiplyScalar(forwardInput).addScaledVector(right, strafeInput)\`. Applying \`(strafe, 0, forward)\` straight to \`position\` is the bug where "the controls feel inverted/mismatched" the moment the camera turns — world +z stops being "up the screen", so WASD no longer matches what the player sees.
- **Mouse-look = yaw + pitch, with pitch CLAMPED.** Accumulate \`yaw -= movementX * s; pitch -= movementY * s\` and clamp pitch to ~[-1.45, 1.45] rad so the camera can never flip / gimbal-lock (no vertical look is the other half of this complaint). Keep sensitivity \`s\` LOW and CONSTANT (~0.002 rad/px) — never multiply mouse deltas by large numbers or by \`dt\`; that is the "way too springy / extreme" feel.
- **Drive the camera from the MOUSE, not the player's heading.** A follow camera that re-aims itself to the player's facing every frame swings unpredictably and re-creates the mismatch. Yaw/pitch come from the pointer; the player turns to face their movement/target independently of the camera.
- **Don't reinvent it.** \`view_game_feel({ name: 'three/camera-controller.jsx' })\` is a vetted controller that does ALL of the above — camera-relative \`moveVector(strafe, forward)\`, clamped mouse-look, smooth (non-springy) follow, and the pointer-lock + drag fallback. Adapt it instead of hand-rolling camera math.
