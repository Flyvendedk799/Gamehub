...
2. **\`choose_engine\`** — emit \`{ engine: 'three' | 'phaser' | 'canvas2d', rationale: 1-sentence }\`. Match to brief AND to the spec from step 1:
   ...
   - **RECOMMENDED SKILLS — \`import_skill\`, do NOT retype.** \`choose_engine\` returns the vetted, engine-correct skills that implement YOUR declared capabilities — e.g. \`enemy-ai\` + \`wave-spawner\` for an escalating shooter, \`level-orchestrator\` + \`save-state\` for progression, \`dialog-flow\` for narrative, \`economy-system\` for a shop/tower-defense, \`procedural-gen\` for roguelikes, \`mobile-controls\` for touch. For each, call **\`import_skill({ name })\`** — it writes the tested module to \`src/engine/\` and returns the import line + its exported functions. **Call \`import_skill\` AFTER you have scaffolded your entry file (steps 5–6), not before** — when \`src/main.js\` already exists, \`import_skill\` wires the \`import\` line straight into it for you; called too early (no entry yet) you must add the import by hand. Then **build each system by CALLING those functions** — e.g. \`const waves = createWaveSystem({ ... });\` then \`waves.update(time, delta)\` in your loop. CALLING the imported skill IS how you implement that system — do NOT then write your own version of it. Re-deriving a wave/enemy/progression/dialogue system by hand is the dominant source of flat, buggy, over-long builds; the imported module is tested AND already wires its \`getState()\` into \`window.__game.debug.snapshot\` for verification, and the quality gate flags a skill imported-but-never-called (\`skill-staged-unused\`). (\`view_game_feel\` is still right for the small inline FEEL snippets — screen-shake, hitstop, particle-burst — which you DO adapt into your handlers.)
