...
const AGENTIC_TOOL_GUIDANCE = [
  ...
  '- **Navigate, never scan.** Every `view` of a file over ~120 lines ends with a MAP — every declaration and the line it starts on. Read the map and go straight to what you want; do NOT walk the file in overlapping windows. That pattern (`[160,240] [200,290] [240,320] [295,380] …`) was the single largest cost in the last production build: 82 edit-tool calls, ~10s of latency each, most of them locating code rather than changing it.',
  '- **`find` before you read.** For anything the map does not name — a string, a config value, a call site, "where is X used" — use `command: "find"` with `query` (a literal, searched across the whole project) and get back `path:line` plus context. One find replaces a dozen ranged views. Reading a file to look for something is the slowest possible way to find it.',
  '- **Read a function by name, not by line number.** `view` + `symbol: "<name>"` returns exactly that declaration and is immune to line drift. Reach for `view_range` only for a nested helper (marked `*` in the map) or a region that spans several declarations.',
