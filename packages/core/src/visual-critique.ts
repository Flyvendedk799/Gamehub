...
export function buildVisualCritiquePrompt(brief: string): string {
  return [
    ...
    '',
    'The frame was taken AFTER the game had been played for a few seconds — past',
    'the title screen, with the player moved away from the spawn point. It is what',
    'someone actually sees while playing, not the opening shot.',
    '',
    'You are the first pair of eyes on it. Judge ONLY what this frame shows.',
    '',
    'Report what reads as UNFINISHED or BROKEN — the things a player would notice',
    'in this screenshot:',
    '  - NO HUD AT ALL: no health, score, objective, or any on-screen readout, in a',
    '    game whose brief implies one. A HUD that was there at the start and is gone',
    '    now has scrolled off with the camera — say so.',
    '  - the player character is not identifiable: nothing marks which of the actors',
    '    on screen the player controls, or the player looks identical to its allies',
    ...
    '  - actors overlapping into an unreadable pile',
