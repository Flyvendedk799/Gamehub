...
// Vite's `?raw` import suffix is unavailable outside the bundler (the cloud
// agent runs under plain Node/tsx), so load the sibling .jsx payloads at
// module init via fs. import.meta.url resolves against this file's location.
const raw = (file: string): string => readFileSync(new URL(file, import.meta.url), 'utf8');
const androidJsx = raw('./android.jsx');
const ipadJsx = raw('./ipad.jsx');
const iphoneJsx = raw('./iphone.jsx');
const macosSafariJsx = raw('./macos-safari.jsx');
const watchJsx = raw('./watch.jsx');
