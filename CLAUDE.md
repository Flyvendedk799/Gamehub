...
## Stack
pnpm + Turbo monorepo. TypeScript everywhere. Next.js (App Router) frontend; Fastify API; Node worker services; Postgres 16; Redis + BullMQ; object storage (S3/R2); Clerk auth; Stripe billing; Playwright browser-worker pool; Cloudflare edge. Tests: Vitest (unit) + Playwright (e2e). Lint/format: Biome. Default to the latest Claude models for generation.
